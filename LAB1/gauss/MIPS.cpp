...
#define RFRESULT_FILENAME "RF_result_bubblesort.txt"
#define DMRESULT_FILENAME "Dmem_result_bubblesort.txt"
#else
...
#ifdef MYCODE
            imem.close();
            string fstr = string("myCode/")+string(INS_FILENAME);
            imem.open(fstr.c_str());
            if (imem.is_open())
            {
                while (getline(imem,line))
                {
...
#ifdef MYCODE
            dmem.close();
            string fstr = string("myCode/")+string(DAT_FILENAME);
            dmem.open(fstr.c_str());
            if (dmem.is_open())
            {
                while (getline(dmem,line))
                {
